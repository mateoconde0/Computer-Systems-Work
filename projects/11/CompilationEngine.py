...
class CompilationEngine:
    def __init__(self,infile,files = None):
        ...
        self.symbols = ['{', '}', '(' , ')' , '[' , ']' , '.' , ',' , ';' , '+' , '-' , '*' ,'/' , '&amp;' , '|' , '&lt;' , '&gt;' , '=' , '~']
        self.ops = ['+','-','*','/','&amp;','|','&lt;','&gt;','=']
        self.classSymbolTable = {'name':[],'type':[],'kind':[],'number':[],'numField':0,'numStatic':0}
        self.subroutineTableStructure = {'name':[],'type':[],'kind':[],'number':[],'numArgument':0,'numLocal':0}
    ...
    def CompileClass(self,loc):
        ...
        while(hasNext):
            ...
            if(currToken[1] == 'field' or currToken[1] == 'static'):
                #we have a new variable --> need to add to the table 
                print(currToken)
                time.sleep(5)
                #add type to the class table 
                self.classSymbolTable['kind'].append(currToken[1])
    ...
    def CompileClassVarDec(self,loc):
        ...
        if(self.data[loc].find('static') != -1 or self.data[loc].find('field') != -1):
            ...
            if(currToken[0] in ['<keyword>','<identifier>']):
                self.classSymbolTable['type'].append(currToken[1])
                outstring.append(self.data[loc])
                loc += 1 
            else: 
            ...
            while(hasNext):
                if(self.data[loc].find(';') != -1):
                    # self.classSymbolTable['number'].append(self.classSymbolTable[])
                    outstring.append(self.data[loc])
                    loc += 1  
                    hasNext = False
                else: 
                    self.classSymbolTable['name'].append(self.data[loc].split(' ')[1])
    ...
    def CompileSubroutineDec(self,loc):
        ...
        if(self.data[loc].find('constructor') != -1  or self.data[loc].find('function') != -1 or self.data[loc].find('method') != -1):
            ...
            outstring.append(self.data[loc])
            loc += 1 
            if(self.data[loc].find('void') != -1 or self.data[loc].find('identifier') != -1 or self.data[loc].find('keyword')): 
                outstring.append(self.data[loc])
                loc += 1 
            else: 
